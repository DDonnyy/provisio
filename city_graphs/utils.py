...
def parse_overpass_route_response(loc, city_crs, boundary):
    ...
    if "node" in route["type"].unique():
        platforms = route[route["type"] == "node"][["lat", "lon"]].reset_index(
            drop=True
        )
...
def get_linestring(route):
    ...
    def get_sequences(connect_ser, sequences=None):
        ...
        if len(next_path) == 0:
            ...
            if branching is None:
                ...
                for path in next_path:
                    series_ = series.drop(
                        [path_ for path_ in next_path if path_ != path]
                    )
    ...
    connect_series = route.apply(
        lambda x: find_connection(x, path_buff), axis=1
    ).dropna()
    ...
    for i in range(len(sequence) - 1):
        ...
        check_reverse = (
            gpd.GeoSeries([Point(line2[0]), Point(line2[-1])])
            .distance(con_point2)
            .idxmin()
        )
        if check_reverse == 1:
...
def project_platforms(loc, city_crs):
    ...
    if platforms is not None:
        ...
        check_reverse = (
            gpd.GeoSeries([Point(line.coords[0]), Point(line.coords[-1])])
            .distance(stops[0])
            .idxmin()
        )
        if check_reverse == 1:
        ...
        condition = (stops_distance > edge_indent) & (
            stops_distance < line_length - edge_indent
        )
        stops, distance = stops[condition].reset_index(drop=True), [0] + list(
            stops_distance[condition]
        )
...
def recursion(stops, threshold):
    ...
    stops["to_del"] = stops.apply(
        lambda x: get_index_to_delete(stops, x, threshold), axis=1
    )

    if stops["to_del"].isna().all():
        return stops["geometry"]
    else:
        stops_near_pair = stops.dropna().apply(
            lambda x: sorted([x.name, x.to_del]), axis=1
        )
...
def get_index_to_delete(other_stops, loc_stop, threshold):
    ...
    stops_to_del = (
        other_stops.geometry.distance(loc_stop.geometry)
        .sort_values()
        .drop(loc_stop.name)
    )
...
def get_nearest_edge_geometry(points, G):
    ...
    G = G.edge_subgraph(
        [
            (u, v, n)
            for u, v, n, e in G.edges(data=True, keys=True)
            if e["type"] == "walk"
        ]
    )
    ...
    edges_geom = pd.DataFrame(
        edges_geom, index=points.index, columns=["edge_id", "edge_geometry"]
    )
...
def add_connecting_edges(G, split_nodes):
    ...
    split_nodes["connecting_node_id"] = list(
        range(start_node_idx, start_node_idx + len(split_nodes))
    )
    nodes_attr = (
